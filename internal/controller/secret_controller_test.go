...
func tlsSecret(name, ns string, ann map[string]string, certPEM, keyPEM []byte) *corev1.Secret {
...
func TestReconcile_SecretWithoutAnnotation_SkipsACM(t *testing.T) {
	...
	require.NoError(t, r.Client.Create(context.Background(),
		tlsSecret("s", "default", nil, certPEM, keyPEM)))

	_, err := r.Reconcile(context.Background(), nn("s", "default"))
	require.NoError(t, err)
...
func TestReconcile_FirstImport_WritesARNAnnotation(t *testing.T) {
	...
	require.NoError(t, r.Client.Create(context.Background(),
		tlsSecret("s", "default", map[string]string{annotations.Enabled: "true"}, certPEM, keyPEM)))
...
func TestReconcile_FingerprintMatch_SkipsImport(t *testing.T) {
	...
	r, _ := buildReconciler(t, m)
	require.NoError(t, r.Client.Create(context.Background(),
		tlsSecret("s", "default", map[string]string{
