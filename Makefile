...
# Download Go modules (run this first)
mod-download:
	docker compose run --rm dev mod download

# Tidy go.mod and go.sum
mod-tidy:
	docker compose run --rm dev mod tidy

# Build the manager binary
build:
	docker compose run --rm -e CGO_ENABLED=0 -e GOOS=linux -e GOARCH=amd64 \
		dev build -o bin/manager ./cmd/manager
...
setup-envtest:
	docker compose run --rm --entrypoint="" dev sh -c \
		'go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use --bin-dir .envtest/bin -p path'
